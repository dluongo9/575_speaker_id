...
def make_lst():
    for lang in LANGS:
        if lang == 'russian':  # TODO take out
            dir = os.path.join('untarred', lang, LANGS[lang])
            ...
            with open('../toy_database/norm/train_world.lst', 'w') as file:  # TODO update in-line with folder/file from FOLDERS
                writer = []
                for i in range(len(validated['path']))[:20]:  # slice to shorten
                    path, client_id = validated['path'][i][:-4], validated['client_id'][i]
                    writer.append(path + '\t' + client_id)
                file.write('\n'.join(writer))

            with open('../toy_database/dev/for_models.lst', 'w') as file:
                writer = []
                for i in range(len(validated['path']))[20:30]:
                    path, client_id = validated['path'][i][:-4], validated['client_id'][i]
                    writer.append(path + '\t' + client_id + '\t' + client_id)
                file.write('\n'.join(writer))

            with open('../toy_database/dev/for_probes.lst', 'w') as file:
                writer = []
                for i in range(len(validated['path']))[30:40]:
                    path, client_id = validated['path'][i][:-4], validated['client_id'][i]
                    writer.append(path + '\t' + client_id)
                file.write('\n'.join(writer))

            with open('../toy_database/eval/for_models.lst', 'w') as file:
                writer = []
                for i in range(len(validated['path']))[40:50]:
                    path, client_id = validated['path'][i][:-4], validated['client_id'][i]
                    writer.append(path + '\t' + client_id + '\t' + client_id)
                file.write('\n'.join(writer))

            with open('../toy_database/eval/for_probes.lst', 'w') as file:
                writer = []
                for i in range(len(validated['path']))[50:60]:
                    path, client_id = validated['path'][i][:-4], validated['client_id'][i]
                    writer.append(path + '\t' + client_id)
                file.write('\n'.join(writer))
